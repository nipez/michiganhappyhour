...
export function renderSpotPage(venue, related = [], opts = {}) {
  ...
  // Use verified deals only so placeholders like "call for current" are not Offers.
  const offers = buildOffers(realDeals);
  ...
  // (Actual business hours, when known, already go in openingHours above.)
  const scheduleProps = [];
  if (daysText) {
...
export function renderSpotMarkdown(venue, related = []) {
  ...
  const hhEnd = venue.hh_end || "";
  const days = parseJsonArray(venue.hh_days);
  const deals = verifiedDeals(parseJsonArray(venue.deals));
  const region = venue.region || "";
  const regionLabel = venue.region_name || REGION_LABELS[region] || town;
  const slug = venueSlug(venue);
  const canonical = `https://michiganhappyhour.com/spots/${slug}`;
  ...
  if (websiteHref) lines.push(`- **Website:** ${websiteHref}`);
